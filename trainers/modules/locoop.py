...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # print(prompts) -> "'X X X X X X X X X X X X X X X X swimming cap.', 'X X X X X X X X X X X X X X X X bath towel.',
    ...
    def forward(self):
        # embedding　空間でpromptを操作
        ctx = self.ctx
        if ctx.dim() == 2:
            ctx = ctx.unsqueeze(0).expand(self.n_cls, -1, -1)

        # register_buffer() に保存されている
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        prompts = self.prompt_learner()  # Tensor
